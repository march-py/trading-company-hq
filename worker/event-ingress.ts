...
export type PersistIdempotentEvent = (
...
function isAuthorized(
  ...
  return (
    ...
    && subtle.timingSafeEqual(supplied, expected)
  );
}
...
async function readBoundedBody(
  ...
  if (declaredLength !== null) {
    ...
    if (
      ...
      || parsedLength < 0
      || parsedLength > MAX_EVENT_BODY_BYTES
    ) {
      return null;
    }
  }
...
export function createEventIngressHandler(
  persist: PersistIdempotentEvent =
    persistIdempotentEvent,
  dispatch: DispatchEvent =
    dispatchEvent,
) {
  return async function handleEventIngress(
    request: Request,
    env: RuntimeEnv,
  ): Promise<Response> {
    if (
    ...
    const stored = await persist(
      {
        ...
        correlation_id:
          validated.value.correlation_id
          ?? requestId,
        request_body_sha256:
          requestBodySha256,
